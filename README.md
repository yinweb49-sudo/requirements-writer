...
一个**与项目解耦**、面向 vibe coding 的 Agent Skill：先帮助普通用户或产品经理把业务结果说清楚；从无到有且有界面时，先用可交互视觉原型建立沟通锚点，再冻结正式需求和技术方案；最后把复杂任务拆成工作量与上下文可控的 AI 执行单元，并按项目策略建立安全的代码恢复锚点。

Skill 会按风险选择轻量、标准或审计模式，把用户原话、代码事实和外部资料整理为可追溯证据，再用“当前决策台账”固定真正指导开发的结论。对于新界面，先形成最小业务草案和可交互原型，让用户基于看得见的页面补充、否定和确认需求；修改闭环后才补齐数据、接口、鉴权、迁移和正式分期。每期限制为一个主要问题，带场景化验收、阶段交接和按授权执行的 Git 检查点。
...
│   ├── visual-first-workflow.md           # 从最小业务草案到可交互原型、版本确认和需求冻结
...
- **视觉先行**（从无到有且有界面时强制）：先写最小业务草案，再用正式前端代码 + 可替换 mock 做可交互原型；用户确认必须绑定提交号或版本、页面入口和截图 / 录屏。必要时先做最小期 0 技术探针，但探针后仍回到视觉原型。视觉修改闭环前不冻结后端、数据和鉴权方案。
...
3. skill 自动读档案 → 选择文档强度 → 建立需求证据与当前决策 → 有新界面时先产出视觉发现草案和可交互原型 → 用户确认后冻结正式需求 → 生成后续 `开发提示词/期N-*.md`。
...
├── <功能名称>-开发需求文档.md      # v0.x 视觉发现草案 → v1.0 已确认开发基线
└── 开发提示词/
    ├── README.md
